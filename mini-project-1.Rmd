...
- `steam_games`: Research question: how different game variables such as price and release date relate to its review.
- `cancer_sample`: Research question: how different dimension measuraments like mean radius of nuclei and mean perimeter of nuclei relate to each other.

I choose to analyze the `steam_games` dataset because it seemes more interesting. Besides, cleaning the `all_reviews` column seems like a good challange.
...
The `all_reviews` column was essentially a sentence containing lots of different information, so I decided to extract the positive review frequency for further analysis.
...
This dataset has a lot of missing values, so it is important to know how they are distributed.
...
All variables but `id` and `url` contain missing values. In particular `recent_reviews` and `mature_content` contain almost 75% of missing values.

4. Work

As stated in the research objetive, I want to understand how the reviews vary as a function of original price and release date.
...
It seems the neab review positiveness decreased through the years but recently has been increasing.
